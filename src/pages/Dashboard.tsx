...
import { User, BookOpen, Clock, CheckCircle, Circle, Calendar, MapPin, DollarSign, GraduationCap } from 'lucide-react';
import { toast } from '@/hooks/use-toast';

const Dashboard: React.FC = () => {
  const { t, dir } = useLanguage();
  const { profile, savedPrograms, applications, updateApplicationStep, removeSavedProgram } = useUser();
  ...
  const handleStepToggle = (programId: string, stepId: string, currentState: boolean) => {
    updateApplicationStep(programId, stepId, !currentState);
    toast({
      title: !currentState ? (dir === 'rtl' ? 'مرحله تکمیل شد! 🎉' : 'Step Completed! 🎉') : (dir === 'rtl' ? 'مرحله به‌روزرسانی شد' : 'Step Updated'),
      description: !currentState 
        ? (dir === 'rtl' ? 'عالی! یک قدم دیگر به هدفت نزدیک شدی' : 'Great progress! You\'re one step closer to your goal')
        : (dir === 'rtl' ? 'وضعیت مرحله به‌روزرسانی شد' : 'Step status has been updated'),
    });
  };

  return (
    <div className={`min-h-screen bg-gray-50 ${dir === 'rtl' ? 'font-vazir' : 'font-inter'}`}>
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">
            {dir === 'rtl' ? `خوش آمدی، ${profile.name}!` : `Welcome back, ${profile.name}!`}
          </h1>
        ...
        <Tabs defaultValue="overview" className="space-y-6">
          <TabsList className={`grid w-full grid-cols-4 ${dir === 'rtl' ? 'font-vazir' : ''}`}>
            <TabsTrigger value="overview">{dir === 'rtl' ? 'نمای کلی' : 'Overview'}</TabsTrigger>
            <TabsTrigger value="saved">{dir === 'rtl' ? 'ذخیره شده' : 'Saved Programs'}</TabsTrigger>
            <TabsTrigger value="applications">{dir === 'rtl' ? 'مراحل اقدام' : 'Application Steps'}</TabsTrigger>
            <TabsTrigger value="profile">{dir === 'rtl' ? 'پروفایل' : 'Profile'}</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="overview" className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {savedPrograms.filter(p => new Date(p.deadline) > new Date()).length}
                  </div>
                </CardContent>
              </Card>
            </div>
                      ...
                      <div className="flex items-start justify-between">
                        ...
                        {program.funding === 'full' && (
                          <Badge className="bg-success text-success-foreground">
                            {dir === 'rtl' ? 'کاملاً تأمین مالی شده' : '100% Funded'}
                          </Badge>
                        )}
                      </div>
                    ...
                    <CardContent>
                      <div className="space-y-4">
                        ...
                        <div className="flex space-x-3 rtl:space-x-reverse">
                          ...
                          >
                            {dir === 'rtl' ? 'حذف از ذخیره شده‌ها' : 'Remove from Saved'}
                          </Button>
                    ...
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <CardDescription>{program.university} • {program.country}</CardDescription>
                        </div>
                        <div className="text-right">
                          <div className="text-2xl font-bold text-primary">{application.progress}%</div>
                    ...
                    <CardContent>
                      <div className="space-y-3">
                        {application.steps.map((step) => (
                          <div key={step.id} className="flex items-start space-x-3 rtl:space-x-reverse p-3 rounded-lg hover:bg-gray-50">
                            <button
                              onClick={() => handleStepToggle(application.programId, step.id, step.completed)}
                              className="mt-0.5"
                            >
                              {step.completed ? (
                            ...
                            <div className="flex-1">
                              ...
                              {step.dueDate && (
                                <div className="flex items-center space-x-1 rtl:space-x-reverse text-xs text-orange-600 mt-2">
                                  ...
                                  <span>{dir === 'rtl' ? 'مهلت:' : 'Due:'} {step.dueDate}</span>
                                </div>
                              )}
                            </div>
                          </div>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center space-x-2 rtl:space-x-reverse">
                  ...
                  <span>{dir === 'rtl' ? 'اطلاعات شخصی' : 'Personal Information'}</span>
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-gray-700">
                    {dir === 'rtl' ? 'کشورهای مورد علاقه' : 'Preferred Countries'}
                  </label>
                  <div className="p-3 bg-gray-50 rounded-lg flex items-center space-x-2 rtl:space-x-reverse">
