...
const ProgramCard: React.FC<ProgramCardProps> = ({
  ...
  const programId = `${title}-${university}`
    .replace(/[^a-zA-Z0-9]/g, "-")
    .toLowerCase();
  ...
  return (
    <div
      className={`bg-white rounded-xl shadow-md border border-gray-200 p-6 card-hover cursor-pointer ${
    ...
    >
      ...
      <div className="flex space-x-3 rtl:space-x-reverse">
        <Button
          ...
          onClick={handleSave}
          className={`flex-1 ${
            isSaved ? "bg-success/10 text-success border-success" : ""
          }`}
        >
          {isSaved
            ? dir === "rtl"
